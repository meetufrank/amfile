...
class Login extends Controller
{
    ...
    public function index()
    {
        ...
        $this->assign('login_pwd', cookie('phone_user_pwd'));
        $this->assign('login_username', cookie('phone_user_tel'));
    ...
    public function doLogin()
    {
        ...
        if(request()->isAjax()){
            ...
            if(empty($userName)){
                return json(['code' => -3, 'data' => '', 'msg' => '手机号不能为空']);
            }
           ...
           $where=[
            ...
            $user = $chatuser->field('id,user_name,pwd,sign,avatar,is_manager,company,tel')
                ->where($where)->find();
...
//             }
            ...
            cookie('phone_user_pwd', $pwd,60*60*24);
            //设置session标识状态
            cookie('phone_user_tel', $user['tel'],60*60*24);
            cookie('phone_user_name', $user['user_name'],60*60*24);
            cookie('phone_user_id', $user['id'],60*60*24);
            cookie('phone_user_sign', $user['sign'],60*60*24);
            cookie('phone_user_avatar', $user['avatar'],60*60*24);
...
//                 }
        ...
        $code= session($tel.'_phone');
         ...
         $data = [
                'user_name' => 'lt_'.mt_rand(999, 9999).date('YmdHis',time()),
                'pwd' => input('post.pwd'),
                'pwd_again'=>input('post.c_pwd'),
