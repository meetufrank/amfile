...
class Service
{
    ...
    public static function submitOrder($amount, $orderid = null, $type = null, $title = null, $notifyurl = null, $returnurl = null, $method = null)
    {
        ...
        $request = request();
        $notifyurl = isset($params['notifyurl']) ? $params['notifyurl'] : request()->root(true) . url('amservice/api/notifyx/type/'.$type);
        $returnurl = isset($params['returnurl']) ? $params['returnurl'] : request()->root(true) . url('amservice/api/returnx',['type'=>$type]);
        $html = '';
    ...
    public static function getConfig($type = 'wechat')
    {
        ...
        if (isset($config['cert_client']) && substr($config['cert_client'], 0, 6) == 'common') {
            $config['cert_client'] = APP_PATH . $config['cert_client'];
        }
        if (isset($config['cert_key']) && substr($config['cert_key'], 0, 6) == 'common') {
            $config['cert_key'] = APP_PATH . $config['cert_key'];
        }
             
        
        $config['notify_url'] = empty($config['notify_url']) ? url('amservice/api/notifyx',['type'=>$type]) : $config['notify_url'];
        $config['notify_url'] = !preg_match("/^(http:\/\/|https:\/\/)/i", $config['notify_url']) ? request()->root(true) . $config['notify_url'] : $config['notify_url'];
        $config['return_url'] = empty($config['return_url']) ? url('amservice/api/returnx',['type'=>$type]) : $config['return_url'];
